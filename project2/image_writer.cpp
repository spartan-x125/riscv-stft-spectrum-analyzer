...
void WriteTargetFrequencySvg(const std::filesystem::path& path,
    ...
    for (std::size_t i = 0; i < values_db.size(); ++i) {
        ...
        const float x = static_cast<float>(left) + time_norm * static_cast<float>(plot_width);
        const float y = static_cast<float>(top + plot_height) - value_norm * static_cast<float>(plot_height);
        out << x << ',' << y << ' ';
    }
    out << "\"/>\n</svg>\n";
}
